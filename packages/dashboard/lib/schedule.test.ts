...
describe('approval slot REST contract', () => {
  ...
  it('converts local summer and winter slots to UTC', () => {
    expect(approvalSlot('2026-07-10T18:30', now, losAngeles)).toBe('2026-07-11T01:30:00.000Z');
    expect(approvalSlot('2026-12-10T18:30', now, losAngeles)).toBe('2026-12-11T02:30:00.000Z');
  });
  it('works for a timezone east of UTC', () => {
    expect(approvalSlot('2026-07-10T18:30', now, 'Asia/Kolkata')).toBe('2026-07-10T13:00:00.000Z');
  });
  it('uses the earlier occurrence when the local clock repeats an hour', () => {
    expect(approvalSlot('2026-11-01T01:30', now, losAngeles)).toBe('2026-11-01T08:30:00.000Z');
  });
  it.each(['2026-02-30T18:30', '2026-03-08T02:30', '2026-13-10T18:30'])(
    'rejects nonexistent local time %s',
    (value) => {
      expect(() => approvalSlot(value, now, losAngeles)).toThrow('does not exist');
    },
  );
  ...
  it('rejects a past or current instant before any mutation is submitted', () => {
    ...
    expect(() => approvalSlot('2026-07-10T18:30', instant, losAngeles)).toThrow('future');
    expect(() => approvalSlot('2026-07-10T18:30', instant + 1, losAngeles)).toThrow('future');
  });
});
