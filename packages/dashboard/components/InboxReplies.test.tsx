...
vi.mock('react', async (original) => ({
  ...
  useState: (initial: unknown) => {
    ...
    return [
      hooks.slots[i],
      (value: unknown) => {
        hooks.slots[i] = typeof value === 'function' ? value(hooks.slots[i]) : value;
      },
    ];
  },
...
function find(value: unknown, type: string, text?: string): Node | undefined {
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      const found = find(item, type, text);
...
function findAll(value: unknown, type: string, result: Node[] = []): Node[] {
  if (!value || typeof value !== 'object') return result;
  if (Array.isArray(value)) {
    for (const item of value) findAll(item, type, result);
    return result;
  }
  const node = value as Node;
  if (node.type === type) result.push(node);
  findAll(node.props?.children, type, result);
  return result;
}
function render(canPrepare = true, canDraft = false) {
  hooks.index = 0;
  return InboxReplies({
    ...scope,
    canPrepare,
    actorUserId: 'actor',
    canDraft,
    llmActorRef: canDraft ? 'grok-roleplayer' : undefined,
    canApprove: canDraft,
  });
}
function click(text: string, canDraft = false) {
  const button = find(render(true, canDraft), 'button', text);
...
async function load(data: unknown[] = [], cursor: string | null = null) {
  ...
  await vi.waitFor(() =>
    expect(find(render(), 'button', 'Load reply history')!.props.disabled).toBe(false),
  );
}
beforeEach(() => {
  hooks.slots = [];
...
it('validates conversation identity, state and delivery receipt instead of trusting a success response', () => {
  ...
  expect(
    isInboxReply({ ...reply, draftSource: 'llm', draftActorRef: 'grok-roleplayer' }, scope),
  ).toBe(true);
  expect(isInboxReply({ ...reply, draftSource: 'llm' }, scope)).toBe(false);
  expect(isInboxReply({ ...reply, approvedByUserId: 'operator-1' }, scope)).toBe(false);
});
...
it('fences double clicks and reuses exact text and key after a lost save response', async () => {
  ...
  hooks.send.mockReturnValueOnce(
  ...
  await vi.waitFor(() =>
    expect(find(render(), 'button', 'Retry saving same reply')!.props.disabled).toBe(false),
  );
  expect(find(render(), 'textarea')!.props.disabled).toBe(true);
  ...
  expect(JSON.parse(hooks.send.mock.calls[0][1].body)).toEqual({
    connectionId: id,
    counterpartUuid: id,
    intentKey: key,
    body: reply.body,
...
it('recovers a lost save by reading the exact intent without sending another mutation', async () => {
  ...
  expect(
    find(render(), 'p', 'Saved reply found in history. Check its status below.'),
  ).toBeDefined();
  expect(hooks.send).toHaveBeenCalledTimes(1);
});
...
it('retains conversation scope on older pages and distinguishes uncertain delivery', async () => {
  ...
  expect(hooks.fetch.mock.calls[1][0]).toContain(
    `connectionId=${id}&counterpartUuid=${id}&cursor=${id}`,
  );
...
it('requires explicit confirmation, fences double clicks and sends only the immutable reply ID', async () => {
  ...
  expect(hooks.send.mock.calls[0]).toEqual([
    `/api/v1/models/${id}/inbox/replies/${id}/send`,
    {
      method: 'POST',
...
it('keeps the exact private LLM draft request across failure and requires approval before send controls appear', async () => {
  await load();
  const draftPrompt = 'Write one warm, concise reply without making promises.';
  const textareas = findAll(render(true, true), 'textarea');
  expect(textareas).toHaveLength(2);
  textareas[0].props.onChange!({ target: { value: draftPrompt } });
  hooks.send.mockResolvedValueOnce(new Response('temporary failure', { status: 503 }));
  click('Generate private LLM draft', true);
  await vi.waitFor(() =>
    expect(find(render(true, true), 'button', 'Retry same private draft')!.props.disabled).toBe(
      false,
    ),
  );
  expect(hooks.send).toHaveBeenCalledOnce();
  const draft = {
    ...reply,
    draftSource: 'llm' as const,
    draftActorRef: 'grok-roleplayer',
    approvedByUserId: null,
    approvedAt: null,
    body: 'Warm reply.',
  };
  hooks.send.mockResolvedValueOnce(Response.json({ data: draft }));
  click('Retry same private draft', true);
  await vi.waitFor(() =>
    expect(find(render(true, true), 'h4', 'LLM draft · Prepared — not sent')).toBeDefined(),
  );
  expect(hooks.send.mock.calls[1][0]).toBe(`/api/v1/models/${id}/inbox/replies/draft`);
  expect(hooks.send.mock.calls[1][2]).toEqual({ idempotencyKey: key, retries: 0 });
  expect(JSON.parse(hooks.send.mock.calls[1][1].body)).toEqual({
    connectionId: id,
    counterpartUuid: id,
    intentKey: key,
  ...
  expect(find(render(true, true), 'button', 'Send prepared reply')).toBeUndefined();
  click('Review and approve draft', true);
  hooks.send.mockResolvedValueOnce(
    Response.json({
      data: { ...draft, approvedByUserId: 'actor', approvedAt: '2026-09-19T00:00:00Z' },
    }),
  );
  click('Confirm human approval', true);
  await vi.waitFor(() =>
    expect(find(render(true, true), 'button', 'Send prepared reply')).toBeDefined(),
  );
});
