  ...
  return (
    <div className="page-stack">
      ...
      <section className="card stack" aria-labelledby="campaigns-heading">
        ...
        {snapshot.campaigns.length === 0 ? <p className="subtle">{t('affiliate.noCampaigns')}</p> : <table><caption className="sr-only">{t('affiliate.campaignTable')}</caption><thead><tr><th>{t('affiliate.campaign')}</th><th>{t('affiliate.partner')}</th><th>{t('affiliate.commission')}</th><th>{t('affiliate.referralToken')}</th><th>{t('affiliate.actions')}</th></tr></thead><tbody>{snapshot.campaigns.map(campaign => { const partner = snapshot.partners.find(item => item.id === campaign.partnerId); return <tr key={campaign.id}><td><strong>{campaign.name}</strong><br /><span className="subtle">/{campaign.slug} · {statusLabel(campaign.status, t)}</span></td><td>{partner?.displayName ?? shortId(campaign.partnerId)}</td><td>{new Intl.NumberFormat(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(campaign.commissionBps / 100)}%</td><td className="mono">{campaign.referralToken}</td><td><button className="btn secondary" type="button" disabled={!!busy} onClick={() => void loadReport(campaign)}>{busy === `report:${campaign.id}` ? t('status.loading') : t('affiliate.viewReport')}</button></td></tr>; })}</tbody></table>}
