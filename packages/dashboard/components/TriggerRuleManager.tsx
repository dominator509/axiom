...
const defaultCondition: TriggerCondition = { metric: 'likes', thresholdMode: 'fixed', threshold: 100 };
...
export default function TriggerRuleManager({ modelId, rules, canEdit }: { modelId: string; rules: TriggerRule[]; canEdit: boolean }) {
  ...
  const formatDateUtc = (value: string) => formatDate(new Date(value), locale, { dateStyle: 'medium', timeStyle: 'short', timeZone: 'UTC' });
  const conditionLabel = (rule: TriggerRule) => rule.condition.thresholdMode === 'learned_p90'
    ? t('automation.learnedP90Summary', { count: formatNumber(rule.condition.minimumSamples ?? 4, locale) })
    : `${t('automation.threshold')} ≥ ${formatNumber(rule.condition.threshold ?? 0, locale)}`;
  ...
  function save() {
    ...
    const fixedThresholdInvalid = condition.thresholdMode !== 'learned_p90'
      && (!Number.isFinite(condition.threshold) || (condition.threshold ?? -1) < 0);
    const learnedSamplesInvalid = condition.thresholdMode === 'learned_p90'
      && (!Number.isInteger(condition.minimumSamples) || (condition.minimumSamples ?? 0) < 4 || (condition.minimumSamples ?? 0) > 1_000);
    if (!trimmed || fixedThresholdInvalid || learnedSamplesInvalid) {
