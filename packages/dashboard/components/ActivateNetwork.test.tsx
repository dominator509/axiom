...
vi.mock('@/lib/mutation', () => ({
  createIdempotencyKey: harness.key,
  mutationFetch: harness.send,
}));
vi.mock('./LocaleProvider', () => ({
  useLocale: () => ({
    t: (key: string, values?: Record<string, string | number>) =>
      ({
        'networkActivation.httpFailed': `Activation not confirmed (HTTP ${values?.status}). Check live status before retrying.`,
        'networkActivation.completed': `Configuration reconciliation completed (${values?.bound} bound, ${values?.skipped} skipped). Check live connection status: completion does not mean the tunnel is healthy.`,
...
function approve() {
  render().input.props.onChange!({ target: { checked: true } });
}
...
beforeEach(() => {
  ...
  harness.key.mockReset().mockReturnValue('intent-1');
});
...
it.each([400, 401, 403, 404, 422])(
  'requires new approval and a new intent after HTTP %i',
  async (status) => {
    harness.send
    ...
    expect(render().button.props.disabled).toBe(true);
    harness.key.mockReturnValue('intent-2');
    approve();
    render().button.props.onClick!();
    await vi.waitFor(() => expect(render().message).toContain('reconciliation completed'));
...
it.each([
  {},
  { data: { status: 'synced', bound: -1, skipped: 0 } },
  { data: { status: 'synced', bound: 1 } },
])('does not report completion for an invalid successful response %j', async (body) => {
  harness.send
    .mockResolvedValueOnce(new Response(JSON.stringify(body)))
    .mockResolvedValueOnce(synced(0, 1));
  approve();
  render().button.props.onClick!();
  await vi.waitFor(() => expect(render().message).toContain('not confirmed'));
  render().button.props.onClick!();
