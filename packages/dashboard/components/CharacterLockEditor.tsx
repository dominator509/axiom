  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage(typeof error.detail === 'string' ? error.detail : t('characterLock.conflict'));
        return;
      }
      ...
      if (data?.id !== modelId || data.characterLockPrompt !== intent.current.prompt
        || data.characterLockVersion !== intent.current.version + 1) throw new Error(t('characterLock.unconfirmed'));
      setPrompt(data.characterLockPrompt); setVersion(data.characterLockVersion);
      intent.current = null;
      setMessage(t('characterLock.saved'));
    } catch {
      setMessage(t('characterLock.unconfirmed'));
    } finally { active.current = false; setBusy(false); }
  }
  return <section aria-label={t('characterLock.editorAria')} className="stack">
    <label htmlFor={`character-lock-${modelId}`}>{t('characterLock.label')}</label>
    ...
    <p>{t('characterLock.description', { version })}</p>
    <button type="button" disabled={busy || conflict || prompt.length > 2000} onClick={() => void save()}>
      {busy ? t('characterLock.saving') : intent.current ? t('characterLock.checkSameSave') : t('characterLock.save')}
    </button>
    {conflict && <button type="button" onClick={() => router.refresh()}>{t('characterLock.reloadProfile')}</button>}
