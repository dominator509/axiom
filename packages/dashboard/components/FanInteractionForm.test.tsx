...
vi.mock('@/lib/mutation', () => ({ createIdempotencyKey: () => 'same-entry', mutationFetch: state.send }));
...
it('matches the existing fan-scoped contract', () => {
  expect(interactionPayload('fan', data({ platform: ' x ', kind: ' comment ', direction: 'inbound', content: ' Hello ' })))
    .toEqual({ fanId: 'fan', platform: 'x', kind: 'comment', direction: 'inbound', content: 'Hello' });
});
it('rejects invalid or oversized fields', () => {
  for (const values of [{ platform: '', kind: 'note', direction: 'inbound' }, { platform: 'x', kind: 'note', direction: 'send-now' }, { platform: 'x', kind: 'note', direction: 'inbound', content: 'x'.repeat(4001) }])
...
it('reuses payload and key after uncertainty and refreshes only on success', async () => {
  const values = new Map([['platform', 'x'], ['kind', 'comment'], ['direction', 'inbound'], ['content', 'Original']]);
  vi.stubGlobal('FormData', class { get(key: string) { return values.get(key) ?? null; } });
