...
export default function OrgSettingsForm({ initial }: { initial: { viralSharing: boolean; publishingEnabled: boolean } }) {
  ...
  async function save() {
    ...
    const body = JSON.stringify({ viralSharing, publishingEnabled });
    ...
    try {
      ...
      const result = await readDashboardJson<{ data?: { viralSharing?: unknown; publishingEnabled?: unknown } }>(response);
      if (result.data?.viralSharing !== viralSharing || result.data?.publishingEnabled !== publishingEnabled) throw new Error('Unconfirmed settings');
