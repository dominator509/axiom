...
vi.mock('@/lib/mutation', () => ({ createIdempotencyKey: () => 'lifecycle-intent', mutationFetch: state.send }));
import ModelLifecycleControls from './ModelLifecycleControls';

afterEach(() => { vi.clearAllMocks(); vi.unstubAllGlobals(); });

type ButtonElement = { props: { onClick: () => Promise<void> } };
type LifecycleElement = { props: { children: [unknown, { props: { children: [unknown, ButtonElement] } }] } };

it('requires confirmation before deactivation and preserves the same intent after uncertainty', async () => {
  const confirm = vi.fn(() => true);
  vi.stubGlobal('window', { confirm });
  state.send.mockResolvedValueOnce(new Response('{}')).mockResolvedValueOnce(new Response(JSON.stringify({ data: { id: 'm1', isActive: false } })));
  const view = ModelLifecycleControls({ model: { id: 'm1', displayName: 'Luna', isActive: true }, canEdit: true }) as unknown as LifecycleElement;
...
it('does not render lifecycle controls to read-only roles', () => {
  expect(ModelLifecycleControls({ model: { id: 'm1', displayName: 'Luna', isActive: false }, canEdit: false })).toBeNull();
});
