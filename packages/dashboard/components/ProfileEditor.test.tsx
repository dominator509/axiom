...
vi.mock('@/lib/mutation', () => ({
  createIdempotencyKey: () => 'profile-intent',
...
it('validates required fields, permits clearing bio, and never edits the character lock or account state', () => {
  ...
  expect(() => profilePayload(form)).toThrow('creator name');
  form.set('displayName', 'Creator');
  form.set('bio', 'x'.repeat(501));
  expect(() => profilePayload(form)).toThrow('500');
});
it('retains original changes and key after an uncertain save and validates the returned profile', async () => {
  ...
  state.send.mockResolvedValueOnce(new Response('{}')).mockResolvedValueOnce(
    new Response(
      JSON.stringify({
        data: {
          id: 'model',
  ...
  const submit = ProfileEditor({
    model: { id: 'model', displayName: 'Old', handle: 'old', bio: 'Old' },
  ...
  expect(state.send).toHaveBeenCalledWith(
    ...
    expect.objectContaining({
      method: 'PATCH',
      body: '{"displayName":"Creator","handle":"creator","bio":"","avatarUrl":null}',
    }),
    { idempotencyKey: 'profile-intent' },
  );
