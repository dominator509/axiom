...
export default function TeamOperationsManager({ modelId, members, shifts, notes, canEdit }: { modelId: string; members: TeamMember[]; shifts: TeamShift[]; notes: TeamNote[]; canEdit: boolean }) {
  ...
  function updateShift(shift: TeamShift, status: string) { void send(`/api/v1/models/${encodeURIComponent(modelId)}/team-shifts/${encodeURIComponent(shift.id)}`, { status }, undefined, 'PATCH'); }
  ...
  return <div className="stack">
    ...
    {shifts.length > 0 && <div className="stack">{shifts.map(shift => <article className="card" key={shift.id}><div className="row" style={{ justifyContent: 'space-between' }}><div><strong>{shift.queue}</strong><p className="subtle">{new Date(shift.startsAt).toLocaleString()} → {new Date(shift.endsAt).toLocaleString()} · {members.find(member => member.id === shift.assigneeUserId)?.email ?? shift.assigneeUserId}</p></div>{canEdit && shift.status !== 'completed' && shift.status !== 'cancelled' && <div className="action-row"><button className="btn secondary" type="button" disabled={busy} onClick={() => updateShift(shift, shift.status === 'scheduled' ? 'active' : 'completed')}>{shift.status === 'scheduled' ? 'Start shift' : 'Complete shift'}</button><button className="btn secondary" type="button" disabled={busy} onClick={() => updateShift(shift, 'cancelled')}>Cancel</button></div>}</div></article>)}</div>}
