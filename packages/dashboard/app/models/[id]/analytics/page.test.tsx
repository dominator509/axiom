...
const mocks = vi.hoisted(() => ({ role: 'model', analytics: vi.fn(), viral: vi.fn(), playbookGuidelines: vi.fn(), uiLocale: vi.fn() }));
vi.mock('@/lib/api', () => ({ getSession: async () => ({ user: { role: mocks.role } }), api: { models: { analytics: mocks.analytics, viral: mocks.viral, playbookGuidelines: mocks.playbookGuidelines }, uiLocale: { get: mocks.uiLocale } } }));
import Page from './page';
beforeEach(() => { mocks.role = 'model'; mocks.analytics.mockReset(); mocks.viral.mockReset(); mocks.playbookGuidelines.mockReset(); mocks.uiLocale.mockReset().mockResolvedValue({ data: { locale: 'en' } }); });
...
it('renders localized labels and locale-aware metrics', async () => {
  ...
  mocks.analytics.mockResolvedValue({ data: { totals: { views: 12345, likes: 2, shares: 0, comments: 1 }, windowDays: 30, perPlatform: [{ platform: 'instagram', views: 12345, likes: 2, shares: 0, comments: 1, engagementRate: 0.125 }], daily: [] } });
  mocks.viral.mockResolvedValue({ data: { totalExemplars: 0 } });
