...
vi.mock('@/lib/server-locale', () => ({ getServerLocale: async () => ({
  ...
  t: (key: string, values?: Record<string, string | number>) => ({
    ...
    'playbook.cadenceSectionAria': 'Weekly playbook cadence',
    'playbook.cadenceWeek': `Current week: ${values?.from ?? ''} through ${values?.to ?? ''} (UTC, Monday–Sunday).`,
    'playbook.cadenceAdvisory': 'Advisory only. Pending posts may not publish. Failed, canceled and uncertain posts do not count toward the plan. No schedule is changed automatically.',
    'playbook.cadenceUnavailable': 'Cadence guidance is unavailable. No adherence conclusion can be drawn.',
    'playbook.cadenceEmpty': 'No cadence guidelines saved for this talent.',
    'playbook.cadenceRevision': `Revision ${values?.revision ?? ''}`,
    'playbook.cadenceCounts': `${values?.published ?? 0} published + ${values?.scheduled ?? 0} pending / ${values?.target ?? 0} posts per week.`,
    'playbook.cadenceNoMinimum': 'No weekly minimum configured.',
    'playbook.cadenceDeficitOne': `Under planned cadence by ${values?.deficit ?? 0} post.`,
    'playbook.cadenceDeficitMany': `Under planned cadence by ${values?.deficit ?? 0} posts.`,
    'playbook.cadenceCovered': 'Planned cadence covered; this is not a publication guarantee.',
    'playbook.cadenceSavedTimes': `Saved posting-time guidance: ${values?.times ?? ''}. Confirm the intended timezone before scheduling.`,
    'playbook.reviewGuidelines': 'Review playbook guidelines',
  }[key] ?? key),
}) }));
