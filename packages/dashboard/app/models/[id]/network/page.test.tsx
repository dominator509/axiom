...
const render = async (searchParams?: { oauth?: string; platform?: string }) => renderToStaticMarkup(await NetworkPage({ params: Promise.resolve({ id: 'model' }), searchParams: Promise.resolve(searchParams) }));
...
it('confirms a successful browser OAuth return without trusting arbitrary query text', async () => {
  expect(await render({ oauth: 'connected', platform: 'fanvue' })).toContain('Fanvue connected successfully.');
  expect(await render({ oauth: 'connected', platform: 'unknown' })).not.toContain('connected successfully');
});
