  ...
  return (
    <div className="page-stack">
      ...
      <section className="stack" aria-label="Application crash reports">
        ...
        <nav className="action-row" aria-label="Crash report status">{['open', 'resolved', 'ignored'].map(value => <Link key={value} href={`/incidents?status=${value}`} aria-current={status === value ? 'page' : undefined}>{value}</Link>)}</nav>
        ...
        <nav className="action-row" aria-label="Crash report pages">
          {cursor && <Link href={`/incidents?status=${status}`}>Latest reports</Link>}
          {crashCursor && <Link href={`/incidents?${new URLSearchParams({ status, crashCursor })}`}>Older reports</Link>}
        </nav>
      </section>
      <h2>Job recovery</h2>
      {error && (
