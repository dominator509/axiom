import { pgTable, uuid, text, timestamp, integer, unique, index, foreignKey } from 'drizzle-orm/pg-core';
import { org } from './org.js';
import { modelProfile } from './model_profile.js';
import { authUser } from './auth_user.js';
import { platformConnection } from './platform_connection.js';

export const inboxReplyIntent = pgTable('inbox_reply_intent', {
  id: uuid('id').primaryKey().defaultRandom(),
  orgId: uuid('org_id').notNull().references(() => org.id, { onDelete: 'cascade' }),
  modelId: uuid('model_id').notNull(), connectionId: uuid('connection_id').notNull(),
  actorUserId: text('actor_user_id').notNull(), counterpartUuid: uuid('counterpart_uuid').notNull(),
  intentKey: uuid('intent_key').notNull(), body: text('body').notNull(),
  state: text('state').$type<'pending' | 'dispatching' | 'sent' | 'rejected' | 'uncertain' | 'cancelled'>().notNull().default('pending'),
  remoteMessageUuid: uuid('remote_message_uuid'), providerStatus: integer('provider_status'),
  ...
  foreignKey({ name: 'inbox_reply_actor', columns: [table.orgId, table.actorUserId], foreignColumns: [authUser.orgId, authUser.id] }),
]);
