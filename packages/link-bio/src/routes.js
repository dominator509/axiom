 ...
 */
router.post('/api/v1/models/:id/linkbio', zValidator('json', enableProviderSchema), async (c) => {
    const modelId = c.req.param('id');
    const { kind, config } = c.req.valid('json');
    try {
        await registry.enable(modelId, kind, config);
...
router.delete('/api/v1/models/:id/linkbio/:kind', async (c) => {
    ...
    const validKinds = ['native', 'fanlynks', 'linktree', 'beacons'];
