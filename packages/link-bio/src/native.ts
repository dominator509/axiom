 ...
 */
export class NativeLinkBioProvider implements LinkInBioProvider {
  readonly kind: ProviderKind = 'native';
  ...
  async getProfile(_modelId: string): Promise<string> {
  ...
  async updateProfile(_modelId: string, config: Record<string, unknown>): Promise<void> {
  ...
  async getAnalytics(_modelId: string): Promise<AnalyticsData[]> {
...
function renderProfilePage(modelId: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
