...
describe('createRouter — POST /chat', () => {
    ...
    it('returns the chat result for a valid request', async () => {
        ...
        expect(gateway.chat).toHaveBeenCalledWith([{ role: 'user', content: 'hi' }], expect.objectContaining({
            model: 'model-1',
            temperature: 0.5,
            maxTokens: 100,
            policy: 'cost',
    ...
    it('works with a minimal body (messages only)', async () => {
        ...
        expect(res.status).toBe(200);
        expect(gateway.chat).toHaveBeenCalledWith([{ role: 'user', content: 'hi' }], expect.objectContaining({ model: undefined, temperature: undefined }));
    });
    it('forwards the egress flag to the gateway (L2.6 egress routing)', async () => {
        ...
        expect(res.status).toBe(200);
        expect(gateway.chat).toHaveBeenCalledWith([{ role: 'user', content: 'hi' }], expect.objectContaining({ egress: true }));
    });
    it('forwards egress:false as false (not undefined) for stream', async () => {
        ...
        expect(res.status).toBe(200);
        expect(gateway.chatStream).toHaveBeenCalledWith([{ role: 'user', content: 'hi' }], expect.objectContaining({ egress: false }));
    });
