 ...
 */
export function buildS0(profile) {
    const lines = [];
    // Persona header
    ...
    const personaMap = {
        professional_friendly: 'Your tone is professional, friendly, and approachable. You connect with your audience while maintaining a polished brand voice.',
        casual: 'You speak naturally, like a friend chatting with followers. Keep it real and relatable.',
        hype: 'You are high-energy and enthusiastic. Every post radiates excitement and positivity.',
        educational: 'You are an expert educator breaking down complex topics into engaging content. Inform and inspire.',
 ...
 */
export function buildS1(platform) {
 ...
 */
export function buildS2(exemplars) {
 ...
 */
export function assemblePrompt(segments) {
    const parts = [];
    if (segments.S0) {
        parts.push(`[SYSTEM]\n${segments.S0}`);
    }
 ...
 */
export function generatePhotoshootPrompts(config) {
    const variants = [];
 ...
 */
export function calculateCourseAdherence(input, weights, minimumThreshold) {
    const w = { ...DEFAULT_WEIGHTS, ...weights };
    const threshold = minimumThreshold ?? DEFAULT_THRESHOLD;
    const overall = w.persona * input.personaConsistency +
