...
const anthropicStyleCompletion = (content, input = 100, output = 40) => ({
    id: 'msg_1',
    type: 'message',
    role: 'assistant',
...
describe('streamMistral', () => {
    it('yields delta content chunks', async () => {
        ...
        const chunks = [];
        for await (const c of streamMistral('sk-test', { model: 'm', messages: [] }))
            chunks.push(c);
...
describe('streamLightning', () => {
    it('yields text deltas from Anthropic SSE events', async () => {
        ...
        const chunks = [];
        for await (const c of streamLightning('lt-test', { model: 'm', messages: [] }))
            chunks.push(c);
...
describe('callGoogle', () => {
    ...
    it('surfaces Gemini error objects as errors', async () => {
        ...
        await expect(callGoogle('AIza-test', { model: 'gemini-flash-latest', messages: [{ role: 'user', content: 'x' }] })).rejects.toThrow('Gemini API error 400: invalid arg');
    });
    it('throws on non-ok response', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ error: 'nope' }, 429)));
        await expect(callGoogle('AIza-test', { model: 'gemini-flash-latest', messages: [] })).rejects.toThrow('Gemini API error 429');
    });
});
...
//# sourceMappingURL=new-providers.test.js.map