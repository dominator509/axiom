...
import { OpenAIProvider, callOpenAI, streamOpenAI, OPENAI_BASE_URL, } from './openai.js';
...
describe('streamOpenAI', () => {
    ...
    it('yields content deltas from SSE data lines and stops at [DONE]', async () => {
        ...
        const chunks = [];
        for await (const c of streamOpenAI('sk-test-456', req))
            chunks.push(c);
...
describe('OpenAIProvider', () => {
    ...
    it('chatStream uses exact usage from a final usage chunk', async () => {
        ...
        expect(done.cost).toBeCloseTo((11 * 2.5 + 22 * 10) / 1e6, 10);
    });
    ...
    it('chatStream skips malformed JSON lines', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['data: garbage', 'data: {"choices":[{"delta":{"content":"ok"}}]}', 'data: [DONE]']));
        const p = new OpenAIProvider('sk-test-456');
        const chunks = [];
        for await (const c of p.chatStream([{ role: 'user', content: 'hi' }]))
            chunks.push(c);
