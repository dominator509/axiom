...
import { VeniceProvider, callVenice, streamVenice, VENICE_BASE_URL, } from './venice.js';
import { VLLMProvider, callVLLM, streamVLLM, VLLM_BASE_URL, } from './vllm.js';
import { callDeepSeek, streamDeepSeek, DEEPSEEK_BASE_URL } from './deepseek.js';
import { callGrok, streamGrok, GROK_BASE_URL } from './grok.js';
...
const openaiStyleCompletion = (content, prompt = 100, completion = 40) => ({
    id: 'c-1',
...
describe('callDeepSeek', () => {
    const req = { model: 'deepseek-chat', messages: [{ role: 'user', content: 'hi' }] };
...
describe('streamDeepSeek', () => {
    ...
    it('yields deltas and stops at [DONE]', async () => {
        ...
        const chunks = [];
        for await (const c of streamDeepSeek('ds-key-111', req))
            chunks.push(c);
...
describe('callGrok / streamGrok', () => {
    const req = { model: 'grok-2-latest', messages: [{ role: 'user', content: 'hi' }] };
    ...
    it('streams deltas', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['data: {"choices":[{"delta":{"content":"x"}}]}', 'data: [DONE]']));
        const chunks = [];
        for await (const c of streamGrok('grok-key-222', req))
            chunks.push(c);
        expect(chunks).toEqual(['x']);
    });
});
describe('callVenice / streamVenice', () => {
    const req = { model: 'llama-3.1-70b', messages: [{ role: 'user', content: 'hi' }] };
    ...
    it('streams deltas and throws ProviderError on stream error', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['data: {"choices":[{"delta":{"content":"v"}}]}', 'data: [DONE]']));
        const chunks = [];
        for await (const c of streamVenice('venice-key-333', req))
            chunks.push(c);
...
describe('callVLLM / streamVLLM', () => {
    const req = { model: 'local-model', messages: [{ role: 'user', content: 'hi' }] };
    ...
    it('streams deltas', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['data: {"choices":[{"delta":{"content":"l"}}]}', 'data: [DONE]']));
        const chunks = [];
        for await (const c of streamVLLM(req))
            chunks.push(c);
...
describe('VeniceProvider', () => {
    ...
    it('chatStream uses exact usage from a usage chunk', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['data: {"usage":{"prompt_tokens":7,"completion_tokens":3,"total_tokens":10}}']));
        const p = new VeniceProvider('venice-key-333');
        const chunks = [];
        for await (const c of p.chatStream([{ role: 'user', content: 'hi' }]))
            chunks.push(c);
...
describe('VLLMProvider', () => {
    ...
    it('chatStream uses exact usage from usage chunk', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['data: {"usage":{"prompt_tokens":5,"completion_tokens":6,"total_tokens":11}}']));
        const p = new VLLMProvider();
        const chunks = [];
        for await (const c of p.chatStream([{ role: 'user', content: 'hi' }]))
            chunks.push(c);
