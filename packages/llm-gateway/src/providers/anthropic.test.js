...
import { AnthropicProvider, callAnthropic, streamAnthropic, ANTHROPIC_BASE_URL, } from './anthropic.js';
...
describe('streamAnthropic', () => {
    ...
    it('yields text from content_block_delta events', async () => {
        ...
        const chunks = [];
        for await (const c of streamAnthropic('ant-key-789', req))
            chunks.push(c);
...
describe('AnthropicProvider', () => {
    ...
    it('chatStream emits a final done chunk when the stream ends without stop events', async () => {
        fetchMock.mockResolvedValueOnce(sseResponse(['event: content_block_delta', 'data: {"type":"content_block_delta","delta":{"text":"z"}}']));
        const p = new AnthropicProvider('ant-key-789');
        const chunks = [];
        for await (const c of p.chatStream([{ role: 'user', content: 'hi' }]))
            chunks.push(c);
