...
beforeEach(() => {
    ...
    fetchMock = vi.fn(async (url) => {
        if (url.includes('api.openai.com'))
            return jsonResponse(openaiCompletion);
        if (url.includes('api.anthropic.com'))
            return jsonResponse(anthropicResponse);
        if (url.includes('api.deepseek.com'))
            return jsonResponse(deepseekCompletion);
        if (url.includes('api.x.ai'))
            return jsonResponse(grokCompletion);
        if (url.includes('api.venice.ai'))
            return jsonResponse(veniceCompletion);
...
describe('LLMGateway — availability and selection', () => {
    ...
    it('throws when no providers are available', async () => {
        ...
        await expect(gw.chat(messages)).rejects.toThrow('No providers available — set at least one API key');
    });
