...
import { alignBlocks, buildS0, buildS1, buildS2, buildS3, assemblePrompt, generatePhotoshootPrompts, calculateCourseAdherence, } from './prompts.js';
const baseProfile = {
    id: 'model-1',
...
describe('buildS0', () => {
    ...
    it('falls back to default tone for unknown persona', () => {
        const s0 = buildS0({ ...baseProfile, persona: 'mystery' });
...
describe('buildS3', () => {
    const task = { modelId: 'model-1', platform: 'instagram' };
...
describe('calculateCourseAdherence', () => {
    ...
    it('respects custom weights', () => {
        const score = calculateCourseAdherence({ personaConsistency: 1, platformRuleCompliance: 0, exemplarSimilarity: 0, taskAlignment: 0 }, { persona: 0.5, platform: 0.5, exemplar: 0, task: 0 });
        expect(score.overall).toBe(0.5);
        expect(score.weights.persona).toBe(0.5);
    });
    it('merges partial weights with defaults', () => {
        const score = calculateCourseAdherence({ personaConsistency: 1, platformRuleCompliance: 1, exemplarSimilarity: 1, taskAlignment: 1 }, { task: 0.5 });
    ...
    it('respects custom minimum threshold', () => {
        const score = calculateCourseAdherence({ personaConsistency: 0.6, platformRuleCompliance: 0.6, exemplarSimilarity: 0.6, taskAlignment: 0.6 }, undefined, 0.5);
