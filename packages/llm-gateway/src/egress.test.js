...
async function loadEgress(planeUrl) {
    vi.resetModules();
...
describe('resolveEgressProxy', () => {
    ...
    it('returns the sidecar proxy URL for a healthy bound model', async () => {
        globalThis.fetch = vi.fn().mockResolvedValue(statusBody([
    ...
    it('returns null for a model not present in the status', async () => {
        globalThis.fetch = vi.fn().mockResolvedValue(statusBody([{ model_id: 'other-model', healthy: true, host_ip: '10.77.0.9' }]));
        const { resolveEgressProxy } = await loadEgress();
        expect(await resolveEgressProxy('gpt-4o')).toBeNull();
    });
    ...
    it('caches the result for the TTL window (one plane call for two lookups)', async () => {
        const fetchMock = vi.fn().mockResolvedValue(statusBody([{ model_id: 'gpt-4o', host_ip: '10.77.0.2', healthy: true }]));
        globalThis.fetch = fetchMock;
        const { resolveEgressProxy } = await loadEgress();
...
describe('buildEgressFetch', () => {
    ...
    it('reuses the same ProxyAgent for the same proxy URL', async () => {
        ...
        const d1 = undiciFetchMock.mock.calls[0][1].dispatcher;
        const d2 = undiciFetchMock.mock.calls[1][1].dispatcher;
        expect(d1).toBe(d2);
        undiciFetchMock.mockClear();
    });
});
//# sourceMappingURL=egress.test.js.map