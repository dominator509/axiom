...
function clearAllKeys() {
    ...
    for (const k of ENV_KEYS)
        delete process.env[k.toLowerCase()];
}
const profile = {
    id: 'model-1',
    displayName: 'Ava',
...
function tokenkillerOpts(overrides = {}) {
    return {
        modelId: 'model-1',
        platform: 'instagram',
        profile,
...
function stubCompletion() {
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(new Response(JSON.stringify(completion), {
        status: 200,
...
describe('chatWithTokenKiller (S0–S3 assembly)', () => {
    ...
    it('assembles S0–S3 segments and sends them as the system message', async () => {
        ...
        const result = await gateway.chatWithTokenKiller([{ role: 'user', content: 'generate for instagram' }], { tokenkiller: tokenkillerOpts(), provider: 'deepseek' });
        ...
        expect(Math.ceil(sys.content.length / 4) % 64).toBe(0);
    });
...
describe('prefix cache-hit ratio (LBI-09, target > 97%)', () => {
    ...
    it('stable prefix (same model/platform/S2) hits across varying S3', async () => {
        ...
        await gateway.chatWithTokenKiller([{ role: 'user', content: 'one' }], { tokenkiller: tokenkillerOpts({ task: { modelId: 'model-1', platform: 'instagram', style: 'studio', outfit: 'a', mood: 'm1' } }), provider: 'deepseek' });
        // Same prefix, different S3 task → prefix-cache HIT.
        await gateway.chatWithTokenKiller([{ role: 'user', content: 'two' }], { tokenkiller: tokenkillerOpts({ task: { modelId: 'model-1', platform: 'instagram', style: 'outdoor', outfit: 'b', mood: 'm2' } }), provider: 'deepseek' });
    ...
    it('different S2 exemplars produce a different prefix key (miss)', async () => {
        stubCompletion();
        const gateway = new LLMGateway();
        const exemplarA = [{ id: 'e1', platform: 'instagram', title: 't', caption: 'c', hashtags: [], viralLabel: 'viral', aiNotes: null, features: { a: 1 }, perfScore: 0.9, label: 'win' }];
        const exemplarB = [{ id: 'e2', platform: 'instagram', title: 't', caption: 'c', hashtags: [], viralLabel: 'weak', aiNotes: null, features: { b: 2 }, perfScore: 0.5, label: 'meh' }];
        await gateway.chatWithTokenKiller([{ role: 'user', content: 'one' }], { tokenkiller: tokenkillerOpts({ exemplars: exemplarA }), provider: 'deepseek' });
        await gateway.chatWithTokenKiller([{ role: 'user', content: 'two' }], { tokenkiller: tokenkillerOpts({ exemplars: exemplarB }), provider: 'deepseek' });
        const stats = gateway.getStats().tokenkiller;
    ...
    it('prefix version bump invalidates the prefix', async () => {
        stubCompletion();
        const gateway = new LLMGateway();
        await gateway.chatWithTokenKiller([{ role: 'user', content: 'one' }], { tokenkiller: tokenkillerOpts({ prefixVersion: 'v1' }), provider: 'deepseek' });
        await gateway.chatWithTokenKiller([{ role: 'user', content: 'two' }], { tokenkiller: tokenkillerOpts({ prefixVersion: 'v2' }), provider: 'deepseek' });
        const stats = gateway.getStats().tokenkiller;
        expect(stats.misses).toBeGreaterThanOrEqual(1);
    });
});
...
//# sourceMappingURL=tokenkiller.test.js.map