...
class TestConnector extends BaseConnector {
    constructor(auth = { accessToken: 'tok' }) {
        super('x', 'Test', 'api', auth);
    }
    capability() {
        return {
            publish: true,
    ...
    post(url, body, headers) {
        return this.apiPost(url, body, headers);
    }
    upload(url, fd, headers) {
        return this.apiUpload(url, fd, headers);
    }
    del(url, headers) {
        return this.apiDelete(url, headers);
    }
    writeLog(level, action, message) {
...
describe('apiGet', () => {
    it('performs an authenticated GET and parses JSON', async () => {
        ...
        const result = await c.get('https://api.example.com/v1/things');
    ...
    it('throws with status details and logs an error on non-ok responses', async () => {
        ...
        const c = new TestConnector();
        await expect(c.get('https://api.example.com/v1/things')).rejects.toThrow('API GET https://api.example.com/v1/things failed: 500 Internal Server Error');
...
describe('apiPost', () => {
    it('POSTs a JSON-serialized body with auth header', async () => {
        ...
        vi.stubGlobal('fetch', fetchMock);
        const c = new TestConnector({ accessToken: 'tok' });
        const result = await c.post('https://api.example.com/v1/create', { name: 'x' });
    ...
    it('throws on non-ok responses', async () => {
        ...
        const c = new TestConnector();
        await expect(c.post('https://api.example.com/v1/create', {})).rejects.toThrow('API POST https://api.example.com/v1/create failed: 400 Bad Request');
    });
    it('propagates network errors', async () => {
...
describe('apiUpload', () => {
    it('uploads FormData with Bearer auth and no JSON content-type', async () => {
        ...
        const result = await c.upload('https://api.example.com/v1/upload', fd);
    ...
    it('throws on non-ok responses', async () => {
        ...
        const c = new TestConnector();
        const fd = new FormData();
        await expect(c.upload('https://api.example.com/v1/upload', fd)).rejects.toThrow('API Upload to https://api.example.com/v1/upload failed: 503 Unavailable');
    });
});
describe('apiDelete', () => {
    it('sends a DELETE request and parses JSON', async () => {
        ...
        vi.stubGlobal('fetch', fetchMock);
        const c = new TestConnector({ accessToken: 'tok' });
        const result = await c.del('https://api.example.com/v1/things/1');
    ...
    it('throws on non-ok responses', async () => {
        ...
        const c = new TestConnector();
        await expect(c.del('https://api.example.com/v1/things/1')).rejects.toThrow('API DELETE https://api.example.com/v1/things/1 failed: 404 Not Found');
    });
});
