...
describe('publish', () => {
    it('creates containers with the right media_type and publishes them', async () => {
        ...
        const create1 = fetchMock.mock.calls[0];
        expect(create1[0]).toBe('https://graph.threads.net/v1.0/threads-user-1/threads');
        expect(JSON.parse(create1[1].body)).toMatchObject({
            media_type: 'IMAGE',
            text: 'Hello Threads',
            media_url: 'https://cdn.example.com/a.jpg',
        });
        const create2 = fetchMock.mock.calls[1];
        expect(JSON.parse(create2[1].body)).toMatchObject({
            media_type: 'VIDEO',
            media_url: 'https://cdn.example.com/b.mp4',
        });
        // Publish calls follow, in container order
        const publish1 = fetchMock.mock.calls[2];
        expect(publish1[0]).toBe('https://graph.threads.net/v1.0/threads-user-1/threads_publish');
        expect(JSON.parse(publish1[1].body)).toMatchObject({ creation_id: 'c1' });
        const publish2 = fetchMock.mock.calls[3];
        expect(JSON.parse(publish2[1].body)).toMatchObject({ creation_id: 'c2' });
    });
