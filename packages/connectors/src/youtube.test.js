...
const UPLOAD_SESSION_URL = 'https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable';
function initResponse(location) {
    return new Response('', {
        status: 200,
...
describe('publish', () => {
    it('runs init → download → PUT upload and returns the video id', async () => {
        ...
        const headers = initInit.headers;
        ...
        expect(putInit.headers['Content-Type']).toBe('video/*');
        expect(putInit.headers['Content-Length']).toBe('9');
    });
    ...
    it('does not append the Shorts tag for long-form videos', async () => {
        ...
        vi.stubGlobal('fetch', fetchMock);
        const c = new YouTubeConnector(AUTH);
        await c.publish(input({ options: { title: 'Long', durationSec: 120, aspectRatio: '9:16', tags: ['fun'] } }));
        const initBody = JSON.parse(fetchMock.mock.calls[0][1].body);
...
describe('fetchMetrics', () => {
    it('parses string statistics into ConnectorMetrics', async () => {
        ...
        expect(init.headers.Authorization).toBe('Bearer yt-token-123');
    });
