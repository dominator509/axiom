...
describe('publish', () => {
    it('creates containers then publishes them, returning the last post id', async () => {
        ...
        const publishInit1 = fetchMock.mock.calls[2];
        expect(publishInit1[0]).toBe('https://graph.facebook.com/v22.0/ig-business-1/media_publish');
        expect(JSON.parse(publishInit1[1].body)).toMatchObject({ creation_id: 'container-1' });
        const publishInit2 = fetchMock.mock.calls[3];
        expect(JSON.parse(publishInit2[1].body)).toMatchObject({ creation_id: 'container-2' });
    });
...
//# sourceMappingURL=instagram.test.js.map