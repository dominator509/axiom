...
describe('publish', () => {
    it('uploads an image (INIT → APPEND → FINALIZE) then creates the tweet', async () => {
        ...
        // 0: download
        const [downloadUrl] = fetchMock.mock.calls[0];
        ...
        expect(initInit.headers.Authorization).toBe('Bearer x-token-123');
        // 2: APPEND
        const [, appendInit] = fetchMock.mock.calls[2];
        const appendForm = appendInit.body;
        ...
        // 3: FINALIZE
        const [, finalizeInit] = fetchMock.mock.calls[3];
        const finalizeForm = finalizeInit.body;
        expect(finalizeForm.get('command')).toBe('FINALIZE');
        expect(finalizeForm.get('media_id')).toBe('m1');
        // 4: tweet
        const [tweetUrl, tweetInit] = fetchMock.mock.calls[4];
        expect(tweetUrl).toBe('https://api.twitter.com/2/tweets');
        expect(JSON.parse(tweetInit.body)).toEqual({
            text: 'Hello from X',
            media: { media_ids: ['m1'] },
        });
    });
    it('publishes a text-only tweet without media uploads', async () => {
        ...
        expect(JSON.parse(init.body)).toEqual({ text: 'Hello from X' });
    });
    it('splits large files into multiple APPEND segments', async () => {
        ...
        const [, append0] = fetchMock.mock.calls[2];
        const [, append1] = fetchMock.mock.calls[3];
        expect(append0.body.get('segment_index')).toBe('0');
        expect(append1.body.get('segment_index')).toBe('1');
    });
    it('polls video processing until succeeded before tweeting', async () => {
        ...
        try {
            ...
            const status1Url = fetchMock.mock.calls[4][0];
            const status2Url = fetchMock.mock.calls[5][0];
    ...
    it('fails the publish when media processing reports a failed state', async () => {
        ...
        try {
            const fetchMock = vi
                ...
                .mockResolvedValueOnce(jsonResponse({ processing_info: { state: 'failed', error: { code: 1, name: 'n', message: 'bad media' } } }));
            vi.stubGlobal('fetch', fetchMock);
            const c = new XConnector(AUTH);
            const promise = c.publish(input({ mediaUrls: ['https://cdn.example.com/video.mp4'] }));
            await vi.advanceTimersByTimeAsync(2_000);
...
describe('fetchMetrics', () => {
    it('maps public_metrics into ConnectorMetrics', async () => {
        ...
        expect(init.headers.Authorization).toBe('Bearer x-token-123');
    });
...
//# sourceMappingURL=x.test.js.map