...
describe('publish', () => {
    it('runs init → download → PUT upload → complete and returns the publish id', async () => {
        ...
        expect(initInit.headers.Authorization).toBe('Bearer tt-token-123');
        // 2: download source
        ...
        expect(uploadInit.headers['Content-Length']).toBe('8');
        // 4: complete
    ...
    it('applies defaults for upload options and privacy level', async () => {
        ...
        const completeBody = JSON.parse(fetchMock.mock.calls[3][1].body);
...
describe('fetchMetrics', () => {
    it('maps video statistics into ConnectorMetrics', async () => {
        ...
        expect(init.headers.Authorization).toBe('Bearer tt-token-123');
    });
    it('throws when the API returns an error object', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ error: { code: 'no_permission', message: 'denied' } })));
        const c = new TikTokConnector(AUTH);
        await expect(c.fetchMetrics('vid-1')).rejects.toThrow('TikTok metrics fetch failed: no_permission — denied');
    });
...
describe('revoke', () => {
    it('verifies the user then logs the disconnect (no server-side revocation)', async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ data: { user: { open_id: 'open-1', display_name: 'testuser' } } }));
        vi.stubGlobal('fetch', fetchMock);
        const c = new TikTokConnector(AUTH);
        await c.revoke();
        const [url, init] = fetchMock.mock.calls[0];
