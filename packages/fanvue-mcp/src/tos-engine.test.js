...
function stubVision(score, category = null, extra = {}) {
...
describe('configuration surface', () => {
    it('defines a threshold for every supported platform', () => {
        for (const platform of Object.keys(PLATFORM_RULES)) {
            expect(typeof DEFAULT_PLATFORM_THRESHOLDS[platform]).toBe('number');
        }
    });
...
describe('evaluate', () => {
    ...
    it('reports caption length and hashtag limit violations', async () => {
        stubVision(0.01, null);
        const engine = new ToSEngine();
        const result = await engine.evaluate({ imageData: '/tmp/img.png', caption: 'a'.repeat(9999), hashtags: Array(100).fill('h') }, ['threads']);
...
//# sourceMappingURL=tos-engine.test.js.map