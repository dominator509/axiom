...
import { PrePostHook } from './prepost.js';
const INPUT = {
...
const RESULT = {
    id: 'r1',
    bundleId: 'b1',
    results: [{ platform: 'tiktok', remoteId: 'r1', state: 'published' }],
...
describe('script registration', () => {
    it('registers, lists, and retrieves scripts', () => {
        const script = { name: 'append-cta', beforePublish: async (i) => i, afterPublish: async () => { } };
    ...
    it('rejects duplicate script names', () => {
        const script = { name: 'dup', beforePublish: async (i) => i, afterPublish: async () => { } };
        hook.registerScript(script);
...
//# sourceMappingURL=prepost.test.js.map