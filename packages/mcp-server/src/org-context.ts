...
export async function orgForModel(modelId: string): Promise<string> {
  const res = await db.execute(sql`SELECT resolve_model_org(${modelId}) AS org_id`);
  const rows = (res?.rows ?? []) as Array<{ org_id: string | null }>;
  const orgId = rows[0]?.org_id;
  if (!orgId) throw new Error(`Model ${modelId} not found`);
  return orgId;
}
 ...
 */
export async function withModelOrg<T>(
  modelId: string,
  fn: (tx: any, orgId: string) => Promise<T> | T,
): Promise<T> {
 ...
 * transaction as MCP tools. Missing or malformed settings fail closed and
 * disable the MCP surface until an explicit enabled state is present.
 */
export async function isModelKillSwitchEnabled(modelId: string): Promise<boolean> {
  return withModelOrg(modelId, async (tx, orgId) => {
    ...
    return rows.length === 0 || rows[0]?.publishing_enabled !== true;
  });
}
