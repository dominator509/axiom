...
import { Tier, createCapabilityToken, authenticateAgent } from './auth.js';
...
describe('McpServer.handleRequest — protocol surface', () => {
    ...
    it('lists tools for listTools and tools/list', async () => {
        ...
        expect(a.result.tools).toHaveLength(3);
        const b = await server.handleRequest({ jsonrpc: '2.0', method: 'tools/list', id: 2 });
        expect(b.result.tools).toHaveLength(3);
    });
...
describe('McpServer.callTool — success paths', () => {
    ...
    it('validates inbox reply requirements through the server', async () => {
        const server = makeServer(Tier.Operator);
        await expect(server.callTool('inbox_manage', { modelId: MODEL, action: 'reply' })).rejects.toThrow('messageId and content are required for reply action');
    });
});
describe('McpServer.callTool — permission and validation failures', () => {
    it('rejects a tool above the agent tier', async () => {
        const server = makeServer(Tier.Viewer);
        await expect(server.callTool('inbox_manage', { modelId: MODEL, action: 'read' })).rejects.toThrow('requires tier "operator", agent has "viewer"');
    });
    it('rejects publishing for an operator', async () => {
        const server = makeServer(Tier.Operator);
        await expect(server.callTool('publishing_post', {
            modelId: MODEL,
            action: 'publish',
            post: { platform: 'x' },
    ...
    it('rejects network_configure below autonomous', async () => {
        const server = makeServer(Tier.Manager);
        await expect(server.callTool('network_configure', { modelId: MODEL, config: {} })).rejects.toThrow('requires tier "autonomous", agent has "manager"');
    });
    it('throws for unknown tools', async () => {
        await expect(makeServer(Tier.Autonomous).callTool('ghost_tool', {})).rejects.toThrow('Unknown tool: ghost_tool');
    });
    it('rejects arguments that fail schema validation', async () => {
        const server = makeServer(Tier.Viewer);
        await expect(server.callTool('analytics_query', { modelId: 'not-a-uuid' })).rejects.toThrow('Invalid arguments for "analytics_query"');
    });
    it('rejects missing required arguments', async () => {
        const server = makeServer(Tier.Viewer);
        await expect(server.callTool('analytics_query', {})).rejects.toThrow('Invalid arguments for "analytics_query"');
    });
    it('rejects a model mismatch with the token scope', async () => {
        const server = makeServer(Tier.Viewer, MODEL);
        await expect(server.callTool('analytics_query', { modelId: '22222222-2222-4222-8222-222222222222' })).rejects.toThrow('Model mismatch');
    });
    it('checks tier before schema validation', async () => {
        const server = makeServer(Tier.Viewer);
        await expect(server.callTool('generation_photoshoot', { prompt: 12345 })).rejects.toThrow('requires tier "operator"');
    });
});
...
describe('createMcpServer', () => {
    ...
    it('throws when authentication fails', () => {
        ...
        expect(() => createMcpServer({ headers: { authorization: 'Bearer bogus' } })).toThrow('Authentication failed');
    });
});
