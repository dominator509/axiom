...
async function writeSyncState(
  ...
  return withOrgContext(connection.orgId, async (tx) => {
    await tx
      .insert(schema.patreonSyncState)
      .values({
        orgId: connection.orgId,
        modelId: connection.modelId,
...
async function saveCampaign(
  ...
  campaign: Awaited<
    ReturnType<
      Awaited<ReturnType<typeof patreonConnectorForConnection>>['connector']['syncCampaign']
    >
  >,
) {
  await withOrgContext(connection.orgId, (tx) =>
    tx
      .insert(schema.patreonCampaign)
      .values({
        orgId: connection.orgId,
        modelId: connection.modelId,
        connectionId: connection.id,
...
async function saveMembers(
  ...
  items: Awaited<
    ReturnType<
      Awaited<ReturnType<typeof patreonConnectorForConnection>>['connector']['syncMembers']
    >
  >['items'],
) {
  await withOrgContext(connection.orgId, async (tx) => {
    for (const member of items) {
      await tx
        .insert(schema.patreonMembership)
        .values({
          orgId: connection.orgId,
          modelId: connection.modelId,
          connectionId: connection.id,
        ...
        .onConflictDoUpdate({
          target: [
            schema.patreonMembership.connectionId,
            schema.patreonMembership.providerMemberId,
          ],
          set: {
            providerCampaignId: member.providerCampaignId,
            tierId: member.tierId ?? null,
...
async function savePosts(
  connection: any,
  items: Awaited<
    ReturnType<Awaited<ReturnType<typeof patreonConnectorForConnection>>['connector']['syncPosts']>
  >['items'],
) {
  await withOrgContext(connection.orgId, async (tx) => {
    for (const post of items) {
      await tx
        .insert(schema.patreonPost)
        .values({
          orgId: connection.orgId,
          modelId: connection.modelId,
          connectionId: connection.id,
  ...
  try {
    ...
    if (!egressBinding)
      return apiError(
        ...
        503,
        statusTitle(503),
        'Patreon token exchange unavailable: model egress binding is unhealthy',
      );
    ...
    if (!accessToken)
      return apiError(c, 502, statusTitle(502), 'Patreon token exchange returned no access token');
    const refreshToken =
    ...
    const externalUserId =
      typeof tokenData.user_id === 'string'
        ? tokenData.user_id
    ...
    return apiError(
      ...
      502,
      statusTitle(502),
      'Patreon token exchange or connection persistence failed',
    );
  }
});
  ...
  const data = await withOrgContext(orgId, async (tx) => {
    const [campaigns, members, posts, sync, webhooks] = await Promise.all([
      tx
        ...
        .from(schema.patreonCampaign)
        .where(eq(schema.patreonCampaign.connectionId, connectionId)),
      tx
        .select({ count: sql<number>`count(*)` })
        .from(schema.patreonMembership)
        .where(eq(schema.patreonMembership.connectionId, connectionId)),
      tx
        .select({ count: sql<number>`count(*)` })
        .from(schema.patreonPost)
        .where(eq(schema.patreonPost.connectionId, connectionId)),
      tx
        .select()
        .from(schema.patreonSyncState)
        .where(eq(schema.patreonSyncState.connectionId, connectionId))
        .orderBy(desc(schema.patreonSyncState.updatedAt)),
      tx
        .select()
        .from(schema.patreonWebhookEvent)
        .where(eq(schema.patreonWebhookEvent.connectionId, connectionId))
        .orderBy(desc(schema.patreonWebhookEvent.receivedAt))
        .limit(1),
    ]);
    return {
...
router.get('/connectors/patreon/data', async (c) => {
  ...
  if (!connectionId) return apiError(c, 400, statusTitle(400), 'connectionId query required');
  if (!resource.success)
    return apiError(c, 400, statusTitle(400), 'resource must be campaign, members or posts');
  const connection = await loadPatreonConnection(orgId, connectionId);
  if (!connection) return apiError(c, 404, statusTitle(404), 'Patreon connection not found');
  const table =
    resource.data === 'campaign'
      ...
      : resource.data === 'members'
        ? schema.patreonMembership
        : schema.patreonPost;
  const rows = await withOrgContext(orgId, (tx) =>
    tx
      .select()
      .from(table)
      .where(eq(table.connectionId, connectionId))
      .orderBy(desc(table.syncedAt))
      .limit(100),
  );
...
router.post('/connectors/patreon/sync', zValidator('json', syncSchema), async (c) => {
  ...
  try {
    const existingState = await withOrgContext(orgId, (tx) =>
      tx
        .select()
        .from(schema.patreonSyncState)
        .where(
    ...
    await withOrgContext(orgId, (tx) =>
      writeAudit(tx, orgId, c.get('userId') ?? 'system', 'patreon.sync', connectionId, {
        resource: body.resource,
        count,
      }),
    );
    ...
    return apiError(
      ...
      502,
      statusTitle(502),
      'Patreon sync failed; no provider payload was returned',
    );
  }
});
...
router.post('/webhooks/patreon/:orgId/:connectionId', async (c) => {
  ...
  try {
    ...
    if (error instanceof RequestBodyTooLargeError)
      return apiError(c, 413, statusTitle(413), 'Patreon webhook body too large');
  ...
  try {
    ...
    const inserted = await withOrgContext(orgId, async (tx) => {
      const rows = await tx
        .insert(schema.patreonWebhookEvent)
        .values({
          orgId,
          modelId: connection.modelId,
          connectionId,
        ...
        .onConflictDoNothing({
          target: [
            schema.patreonWebhookEvent.connectionId,
            schema.patreonWebhookEvent.providerEventId,
          ],
        })
        .returning({ id: schema.patreonWebhookEvent.id });
      if (rows.length > 0) {
        await writeAudit(tx, orgId, 'patreon:webhook', 'patreon.webhook', rows[0].id, {
    ...
    return apiError(
      ...
      502,
      statusTitle(502),
      'Patreon webhook processing failed; provider should retry',
    );
  }
});
