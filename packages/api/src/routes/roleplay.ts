...
const actorSchema = z
  .object({ type: z.enum(['human', 'llm']), ref: z.string().trim().min(1).max(128) })
  .strict();
...
const handoffBodySchema = z
  ...
  .strict();
const personaBodySchema = z
  .object({
    expectedRevision: z.number().int().min(0).max(2_147_483_646),
    sourceRef: z
      .string()
      .regex(/^soul\.md(?::[A-Za-z0-9._-]{1,128})?$/)
      .default('soul.md'),
...
export { roleplayGateway } from '../roleplay-runtime.js';
...
function validActorRef(ref: string): boolean {
  return (
    ...
    !/[\r\n\\/]/u.test(ref) &&
    !ref.includes('..') &&
    !ref.includes(String.fromCharCode(0))
  );
}
...
async function actorShift(
  ...
  const actorCondition =
    actor.type === 'human'
      ? and(
          eq(schema.teamShift.assigneeType, 'human'),
          eq(schema.teamShift.assigneeUserId, actor.ref),
        )
      : and(
          eq(schema.teamShift.assigneeType, 'llm'),
          eq(schema.teamShift.assigneeAgentRef, actor.ref),
        );
  const [shift] = await tx
    .select({
      id: schema.teamShift.id,
      queue: schema.teamShift.queue,
      actorType: schema.teamShift.assigneeType,
      actorRef:
        actor.type === 'human'
          ? schema.teamShift.assigneeUserId
          : schema.teamShift.assigneeAgentRef,
    })
    ...
    const [permission] = await tx
      ...
      .where(
        and(
          ...
          eq(schema.agentPermission.agentRef, actor.ref),
...
router.get('/models/:modelId/roleplay', async (c) => {
  ...
  if (!orgId || !userId)
    return apiError(c, 401, statusTitle(401), 'authenticated workspace required');
  if (!uuid.safeParse(modelId).success)
    return apiError(c, 400, statusTitle(400), 'valid model required');
  const actor = actorFromQuery(c, userId);
  const conversationKey = c.req.query('conversationKey') ?? 'default';
  if (!actor || !conversationKeySchema.safeParse(conversationKey).success)
    return apiError(c, 400, statusTitle(400), 'valid actor and conversation key required');
  if (actor.type === 'llm' && !managementRoles.has(c.get('role') ?? ''))
    return apiError(c, 403, statusTitle(403), 'LLM roleplay state is owner-managed');
  const result = await withOrgContext(orgId, async (tx) => {
    ...
    const turns = await tx
      ...
      .orderBy(desc(schema.roleplayTurn.createdAt), desc(schema.roleplayTurn.id))
      .limit(20);
    return {
      ...
      memory: memory
        ...
        .map((row: any) => ({
          ...
          role: row.role,
          speaker: { type: row.speakerType, ref: row.speakerRef },
          content: row.content,
        })),
      ...
      meta: {
        nextSequence: (memory[0]?.sequence ?? 0) + 1,
  ...
  if (result === 'actor')
    return apiError(c, 404, statusTitle(404), 'actor assignment or active shift unavailable');
...
router.post('/models/:modelId/roleplay/memory', async (c) => {
  ...
  try {
    ...
    if (error instanceof RequestBodyTooLargeError)
      return apiError(c, 413, statusTitle(413), 'memory body too large');
    payload = {};
  }
  ...
  if (parsed.data.speaker.type === 'llm' && !managementRoles.has(c.get('role') ?? ''))
    return apiError(c, 403, statusTitle(403), 'LLM memory is owner-managed');
  const result = await withOrgContext(orgId, async (tx) => {
    if (!(await modelReadable(tx, orgId, modelId, c.get('role'), userId))) return null;
    const shift = await actorShift(tx, orgId, modelId, parsed.data.speaker);
    if (!shift) return 'actor' as const;
    const [row] = await tx
      .insert(schema.roleplayMemoryTurn)
      .values({
        orgId,
        modelId,
        ...
        speakerType: parsed.data.speaker.type,
        speakerRef: parsed.data.speaker.ref,
    ...
    if (row)
      await writeAudit(tx, orgId, userId, 'roleplay.memory.append', row.id, {
        modelId,
  ...
  if (result === 'conflict')
    return apiError(
      ...
      409,
      statusTitle(409),
      'memory sequence already exists; reload the roleplay context',
    );
  return c.json(
    {
      data: {
        ...
        role: result.role,
        speaker: { type: result.speakerType, ref: result.speakerRef },
...
router.put('/models/:modelId/roleplay/persona', async (c) => {
  ...
  if (!orgId || !userId)
    return apiError(c, 401, statusTitle(401), 'authenticated workspace required');
  if (!managementRoles.has(c.get('role') ?? ''))
    return apiError(
      c,
      403,
      statusTitle(403),
      'persona editing requires an owner, manager or operator role',
    );
  let payload: unknown;
  try {
    payload = await readBody(c);
    ...
    if (error instanceof RequestBodyTooLargeError)
      return apiError(c, 413, statusTitle(413), 'persona body too large');
    payload = {};
  }
  ...
  const result = await withOrgContext(orgId, async (tx) => {
    ...
    const [row] = await tx
      ...
      .returning();
    if (row)
      await writeAudit(tx, orgId, userId, 'roleplay.persona.save', row.id, {
        modelId,
        source: row.source,
  ...
  if (result === 'conflict')
    return apiError(
      ...
      409,
      statusTitle(409),
      'persona changed since you opened it; reload before saving',
    );
  return c.json(
    {
      data: {
...
router.put('/models/:modelId/roleplay/handoff', async (c) => {
  ...
  try {
    ...
    if (error instanceof RequestBodyTooLargeError)
      return apiError(c, 413, statusTitle(413), 'handoff body too large');
    payload = {};
  }
  const parsed = handoffBodySchema.safeParse(payload);
  if (!parsed.success)
    return apiError(c, 400, statusTitle(400), 'invalid roleplay handoff envelope');
  let handoff: RoleplayHandoff;
  try {
    if (
      !parsed.data.handoff ||
      typeof parsed.data.handoff !== 'object' ||
  ...
  if (actor.type === 'llm' && !managementRoles.has(c.get('role') ?? ''))
    return apiError(c, 403, statusTitle(403), 'LLM handoffs are owner-managed');
  const result = await withOrgContext(orgId, async (tx) => {
    if (!(await modelReadable(tx, orgId, modelId, c.get('role'), userId))) return null;
    ...
    const [current] = await tx
      ...
      .where(
        and(
          eq(schema.roleplayHandoff.orgId, orgId),
          eq(schema.roleplayHandoff.modelId, modelId),
          eq(schema.roleplayHandoff.conversationKey, parsed.data.conversationKey),
        ),
      )
      .limit(1)
      .for('update');
    ...
    const [row] = current
      ? await tx
          .update(schema.roleplayHandoff)
          .set(values)
          .where(eq(schema.roleplayHandoff.id, current.id))
          .returning()
      : await tx
          .insert(schema.roleplayHandoff)
          .values({ orgId, modelId, conversationKey: parsed.data.conversationKey, ...values })
          .returning();
    if (row)
      await writeAudit(tx, orgId, userId, 'roleplay.handoff.save', row.id, {
        modelId,
        conversationKey: row.conversationKey,
  ...
  if (result === 'actor')
    return apiError(c, 404, statusTitle(404), 'actor assignment or active shift unavailable');
  if (result === 'shift')
    return apiError(c, 409, statusTitle(409), 'handoff actor does not match the active shift');
  if (result === 'conflict')
    return apiError(
      c,
      409,
      statusTitle(409),
      'handoff changed since you opened it; reload before saving',
    );
  return c.json(
    { data: { revision: result.revision, handoff: result.payload } },
    parsed.data.expectedRevision === 0 ? 201 : 200,
  );
});
...
function providerFailure(error: unknown): {
  ...
  if (status >= 400 && status < 500) {
    return {
      state: 'rejected',
      status,
      errorCode: status === 401 ? 'provider-auth-required' : 'provider-rejected',
      message: 'roleplay provider rejected the turn; no reply was published',
    };
  }
  return {
    state: 'uncertain',
    status: 503,
    errorCode: 'provider-uncertain',
    message: 'roleplay provider outcome is uncertain; reconcile the turn before retrying',
  };
}
...
router.post('/models/:modelId/roleplay/turn', async (c) => {
  ...
  if (!managementRoles.has(role ?? ''))
    return apiError(c, 403, statusTitle(403), 'roleplay provider turns are owner-managed');
  if (!uuid.safeParse(modelId).success)
    return apiError(c, 400, statusTitle(400), 'valid model required');
  let payload: unknown;
  try {
    ...
    if (error instanceof RequestBodyTooLargeError)
      return apiError(c, 413, statusTitle(413), 'roleplay turn body too large');
    payload = {};
  }
  ...
  if (parsed.data.actor.type !== 'llm')
    return apiError(c, 409, statusTitle(409), 'provider turns require an assigned LLM actor');
  ...
  const prepared = await withOrgContext(orgId, async (tx) => {
    ...
    const [existing] = await tx
      ...
      .where(
        and(
          eq(schema.roleplayTurn.orgId, orgId),
          eq(schema.roleplayTurn.modelId, modelId),
          eq(schema.roleplayTurn.intentKey, parsed.data.intentKey),
        ),
      )
      .limit(1);
    if (existing) {
      if (
        existing.conversationKey !== parsed.data.conversationKey ||
        existing.actorType !== parsed.data.actor.type ||
        existing.actorRef !== parsed.data.actor.ref ||
        existing.input !== parsed.data.content
      )
    ...
    if (
      handoff.actor.type !== parsed.data.actor.type ||
      handoff.actor.ref !== parsed.data.actor.ref
    )
      return 'handoff' as const;
    ...
    const memoryRows = await tx
      ...
      .where(
        and(
          eq(schema.roleplayMemoryTurn.orgId, orgId),
          eq(schema.roleplayMemoryTurn.modelId, modelId),
          eq(schema.roleplayMemoryTurn.conversationKey, parsed.data.conversationKey),
        ),
      )
      .orderBy(desc(schema.roleplayMemoryTurn.sequence))
      .limit(ROLEPLAY_LIMITS.memoryTurns);
    const persona: RoleplayPersonaSnapshot | null = personaRow
      ...
      : null;
    const memory: RoleplayMemoryTurn[] = memoryRows
      .reverse()
    ...
    const [row] = await tx
      ...
      .values({
        ...
        actorRef: parsed.data.actor.ref,
        shiftId: shift.id,
        provider: 'grok',
      ...
      .returning();
    if (row)
      await writeAudit(tx, orgId, userId, 'roleplay.turn.prepare', row.id, {
        modelId,
        actorType: row.actorType,
        provider: row.provider,
      });
  ...
  if (prepared === 'actor')
    return apiError(c, 404, statusTitle(404), 'LLM assignment or active shift unavailable');
  if (prepared === 'handoff')
    return apiError(
      c,
      409,
      statusTitle(409),
      'roleplay handoff is missing, stale or assigned to another actor',
    );
  if (prepared === 'conflict')
    return apiError(
      ...
      409,
      statusTitle(409),
      'intent key already belongs to different roleplay content',
    );
  if (prepared.kind === 'existing') {
    if (prepared.row.state === 'completed') return c.json(roleplayTurnResponse(prepared.row), 200);
    if (prepared.row.state === 'uncertain')
      return apiError(
        c,
        503,
        statusTitle(503),
        'roleplay provider outcome is uncertain; reconcile the turn before retrying',
      );
    if (prepared.row.state === 'rejected')
      return apiError(
        c,
        409,
        statusTitle(409),
        'roleplay turn was rejected; create a new intent after correcting the issue',
      );
    return apiError(c, 409, statusTitle(409), 'roleplay turn is pending; do not retry');
  }

  let result: Awaited<ReturnType<typeof roleplayGateway.chat>>;
  try {
    ...
    if (!result.content.trim() || result.content.length > 8_000)
    ...
    await withOrgContext(orgId, async (tx) => {
      await tx
        ...
        .where(
          and(
            eq(schema.roleplayTurn.id, prepared.row.id),
            eq(schema.roleplayTurn.state, 'pending'),
          ),
        );
      await writeAudit(tx, orgId, userId, `roleplay.turn.${failure.state}`, prepared.row.id, {
        modelId,
        provider: 'grok',
        providerStatus: failure.status,
        errorCode: failure.errorCode,
  ...
  try {
    const finalized = await withOrgContext(orgId, async (tx) => {
      ...
      const [tail] = await tx
        .select({
          maxSequence: sql<number>`coalesce(max(${schema.roleplayMemoryTurn.sequence}), 0)`,
        })
        .from(schema.roleplayMemoryTurn)
        .where(
          and(
            eq(schema.roleplayMemoryTurn.orgId, orgId),
            eq(schema.roleplayMemoryTurn.modelId, modelId),
            eq(schema.roleplayMemoryTurn.conversationKey, prepared.row.conversationKey),
          ),
        );
      ...
      await tx.insert(schema.roleplayMemoryTurn).values([
        {
          orgId,
          modelId,
          conversationKey: prepared.row.conversationKey,
          sequence: nextSequence,
          role: 'user',
          speakerType: 'human',
          speakerRef: 'conversation',
          content: prepared.row.input,
        },
        {
          orgId,
          modelId,
      ...
      const [row] = await tx
        ...
        .returning();
      if (!row) return null;
      await writeAudit(tx, orgId, userId, 'roleplay.turn.completed', row.id, {
        modelId,
        provider: row.provider,
    ...
    if (!finalized)
      return apiError(
        ...
        503,
        statusTitle(503),
        'roleplay result could not be durably recorded; reconcile the turn before retrying',
      );
    ...
    await withOrgContext(orgId, async (tx) => {
      await tx
        ...
        .set({
          state: 'uncertain',
          providerStatus: 503,
          errorCode: 'persistence-uncertain',
          finalizedAt: sql`clock_timestamp()`,
        })
        .where(
          and(
    ...
    return apiError(
      ...
      503,
      statusTitle(503),
      'roleplay result persistence is uncertain; reconcile the turn before retrying',
    );
  }
});
