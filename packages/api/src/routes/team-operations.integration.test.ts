...
describe.skipIf(!url)('team operations in PostgreSQL', () => {
  ...
  it('allows creator preparation only for assigned talent and never creates publication work', async () => {
    ...
    const stageBody = { modelId: models[0], assetId: assets[0], captions: { x: 'A ceramic vase' }, state: 'approved', tosReport: { verdict: 'pass' } };
    ...
    const stageResult = await staged.json() as { data: { id: string; state: string; tosReport: { verdict: string } } };
