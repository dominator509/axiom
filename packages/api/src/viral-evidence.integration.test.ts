...
import { publishedExemplarEvidence } from './routes/viral.js';
...
describe.skipIf(!url)('published insights evidence in PostgreSQL', () => {
  ...
  it('excludes legacy, manual, mismatched, unpublished and future observations without multiplying polls', async () => {
    ...
    await expect(db.transaction(async tx => {
      ...
      const ids: string[] = [];
      const cases = ['valid', 'legacy', 'manual', 'remote-mismatch', 'platform-mismatch', 'pending', 'future-metric', 'future-publication'];
      for (const kind of cases) {
        ...
        await tx.insert(schema.viralExemplar).values({ id, orgId, modelId, bundleId, platform: 'instagram', label: 'viral', perfScore: 2, features: kind === 'legacy' ? {} : { evidence_source: 'published-provider-snapshot-v2' }, embedding: Array(768).fill(0) });
      ...
      expect(rows.filter(row => ids.includes(row.id))).toEqual([{ id: ids[0] }]);
      throw rollback;
