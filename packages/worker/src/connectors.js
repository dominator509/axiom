...
import { InstagramConnector, TikTokConnector, YouTubeConnector, XConnector, FacebookConnector, RedditConnector, ThreadsConnector, DiscordConnector, TelegramConnector, SnapchatConnector, FanvueConnector, } from '@axiom/connectors';
/** Register every real connector; a missing token is a valid (closed) state. */
export function registerConnectors(env = process.env) {
    const token = (v) => v ?? '';
