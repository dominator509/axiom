...
export const publishTarget = async (ctx) => {
    ...
    if (!targetId)
        throw new Error('publish.target: payload.targetId required');
    ...
    const targets = await tx
        ...
        .limit(1);
    if (targets.length === 0)
        throw new Error(`publish.target: target ${targetId} not found`);
    ...
    const bundles = await tx
        ...
        .limit(1);
    if (bundles.length === 0)
        throw new Error(`publish.target: bundle ${target.bundleId} not found`);
    ...
    const models = await tx
        ...
        .limit(1);
    if (models.length === 0)
        throw new Error(`publish.target: model ${bundle.modelId} not found`);
    const model = models[0];
    // 2. Idempotency ledger (L3.4 §4).
    const idemKeyHex = target.idemKey
        ? Buffer.from(target.idemKey).toString('hex')
        : null;
    if (idemKeyHex) {
        ...
        if (ledger.length > 0) {
            ...
            const stored = (ledger[0].responseHash ?? '');
            await tx
    ...
    const caption = bundle.captions?.[platform] ?? '';
