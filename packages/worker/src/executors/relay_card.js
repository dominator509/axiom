...
export const relayCard = async (ctx) => {
    const { tx, job, killSwitchEnabled } = ctx;
    const payload = (job.payload ?? {});
    const bundleId = payload.bundleId;
    if (!bundleId)
        throw new Error('relay.card: payload.bundleId required');
    ...
    const bundles = await tx
        ...
        .limit(1);
    if (bundles.length === 0)
        throw new Error(`relay.card: bundle ${bundleId} not found`);
    const bundle = bundles[0];
    // Resolve the model's relay binding (which channel receives cards).
    const bindings = await tx
        .select()
        .from(schema.relayBinding)
        .where(and(eq(schema.relayBinding.modelId, bundle.modelId), eq(schema.relayBinding.enabled, true), ...(payload.channel ? [eq(schema.relayBinding.channel, payload.channel)] : [])))
        .limit(1);
    ...
    const tosReport = bundle.tosReport ?? {};
    ...
    const content = {
        ...
        hashtagSets: { [binding.channel]: bundle.hashtags ?? [] },
        tosScores: Object.fromEntries(Object.entries(tosReport.scores ?? []).map(([, s]) => [
            s.platform,
            s.score / 100,
        ])),
...
//# sourceMappingURL=relay_card.js.map