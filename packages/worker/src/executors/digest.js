...
export const digestWeekly = async (ctx) => {
    const { tx, job } = ctx;
  ...
  `);
    const agg = Array.isArray(aggRows) ? aggRows[0] : aggRows.rows?.[0];
    ...
    LIMIT 1
  `);
    const topRow = Array.isArray(topRows) ? topRows[0] : topRows.rows?.[0];
  ...
  `);
    const labelRow = Array.isArray(labelRows) ? labelRows[0] : labelRows.rows?.[0];
    const digest = {
    ...
    const description = `${digest.posts} posts · ${digest.views.toLocaleString()} views · ` +
