...
export function evaluateTextToS(caption, hashtags, platforms) {
    const scores = [];
...
export const tosScan = async (ctx) => {
    const { tx } = ctx;
    const payload = (ctx.job.payload ?? {});
    const bundleId = payload.bundleId;
    if (!bundleId)
        throw new Error('tos.scan: payload.bundleId required');
    const bundles = await tx
        ...
        .limit(1);
    if (bundles.length === 0)
        throw new Error(`tos.scan: bundle ${bundleId} not found`);
