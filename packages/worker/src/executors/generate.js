...
import { generatePhotoshootPrompts, buildS0, buildS1, buildS3, assemblePrompt, LLMGateway, } from '@axiom/llm-gateway';
export const contentGenerate = async (ctx) => {
    const { tx, job } = ctx;
    const payload = (job.payload ?? {});
    const modelId = payload.modelId;
    if (!modelId)
        throw new Error('content.generate: payload.modelId required');
    const models = await tx
        ...
        .limit(1);
    if (models.length === 0)
        throw new Error(`content.generate: model ${modelId} not found`);
    ...
    if (payload.enrichWithLlm) {
        try {
            ...
            const chat = await gateway.chat([{ role: 'system', content: prompt }, { role: 'user', content: variants[0].prompt }], { model: payload.model });
            caption = chat.content.trim();
        }
        catch (err) {
            // Best-effort enrichment; prompt engine output still forms the bundle.
            console.error('content.generate enrich failed:', err.message);
        }
    }
