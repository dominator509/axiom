...
export function labelForZ(z) {
    if (z >= LABEL_THRESHOLDS.viral)
        return 'viral';
    if (z >= LABEL_THRESHOLDS.strong)
        return 'strong';
    if (z >= LABEL_THRESHOLDS.baseline)
        return 'baseline';
    return 'weak';
}
export const viralLabel = async (ctx) => {
    const { tx, job } = ctx;
    const payload = (job.payload ?? {});
    const targetId = payload.targetId;
    if (!targetId)
        throw new Error('viral.label: payload.targetId required');
    const targets = await tx
        ...
        .limit(1);
    if (targets.length === 0)
        throw new Error(`viral.label: target ${targetId} not found`);
    ...
    const bundles = await tx
        ...
        .limit(1);
    if (bundles.length === 0)
        throw new Error(`viral.label: bundle ${target.bundleId} not found`);
    ...
    const history = await tx
        ...
        .where(and(eq(schema.contentBundle.modelId, bundle.modelId), eq(schema.postMetric.platform, target.platform), gte(schema.postMetric.collectedAt, windowStart)))
    ...
    const values = history.map((h) => h.engagementRate);
    ...
    const existing = await tx
        ...
        .where(and(eq(schema.viralExemplar.modelId, bundle.modelId), eq(schema.viralExemplar.bundleId, bundle.id), eq(schema.viralExemplar.platform, target.platform)))
        .limit(1);
