...
OPEN_CONTROL_TASK_STATE: `OPEN — stale-worker stop control pending`
OPEN_CONTROL_TASK_REPLY_WIRE: `NONE`
OPEN_CONTROL_TASK_RECEIPT_WIRE: `NONE`
OPEN_CONTROL_TASK_RECEIPT_SHA256: `NONE`
OPEN_CONTROL_TASK_NEXT_ACTION: `Hermes returns one fresh ACK/READ or terminal NACK/BLOCKED. Codex then receipts it and opens one bounded product lane; no duplicate task or stale-lane resume.`
...
LOOP_CURRENT_ACTION: `Await the stale-worker stop ACK/READ or terminal NACK/BLOCKED for the exact task wire; receipt it by logical SEQ, then issue exactly one fresh bounded architecture feature TASK against a freshly read branch tip; accept no stale product lane`

CURRENT_MILESTONE: `M733 — detected stale Hermes product worker and issued superseding stop control; no product implementation lane active`
...
CURRENT_MILESTONE_OPEN: `Hermes must stop the stale worker and return one correlated ACK/READ or terminal NACK/BLOCKED; after Codex receipts the reply, select exactly one finite architecture feature lane; deployment/readiness gates remain separate`
CURRENT_MILESTONE_REMOTE_READBACK_CURRENT: `Stop-control task envelope SHA 87df0e32071072650d1d7075d9a6c77ca2bdc1930e6fff16d091625765c279fa; remote copy and reply pending; no feature or live action`
