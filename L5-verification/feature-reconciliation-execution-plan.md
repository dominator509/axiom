...
- [ ] Full architecture gate: L1 F-15 caption/teaser variants and F-16 automatic winner promotion now have source paths, including published-post attribution, creation-fixed automatic evaluation, frozen evidence, audit, and replay-safe winner reward integration (M242–M264). M900 adds selected-guidance attribution from server-verified guidance receipts and assignment outcomes, with sample-weighted metrics and a mounted dashboard surface. These are not yet deployed/browser-accepted. Manual self-reported outcomes remain excluded from verified learning. Richer hook/timing behavior, statistical/runtime/browser acceptance and provider acceptance remain open; keep this gate open until those requirements are satisfied.
...
- Caption/teaser candidates now flow through owned review bundles, assignment-bound publication attribution, immutable publication snapshots, automatic evaluation and winner rewards (M243–M264). M900 adds selected-guidance attribution from verified receipts and assignment outcomes; remaining work includes richer hook/timing behavior, statistical/runtime/browser acceptance and deployed acceptance. Candidate creation alone is not the completion criterion.
...
- Define the shared F-89 locale contract for `en`, `es`, `ja`, `it`, `pt-BR` and `de`; normalize BCP-47 tags and keep UI locale separate from creator/model content locale. The source slice now consumes that contract in the dashboard shell/navigation/settings, authenticated login hero/form/error/session copy, server-rendered assigned-shift access/empty/error/pagination/handoff copy with `Intl` UTC formatting, reusable team-shift controls and TeamOperationsManager labels/errors/roles/notes with locale-aware UTC timestamps, CalendarBoard scheduled-time display, and mobile dashboard/selector; the authenticated dashboard shell also localizes workspace, home, role, pending-access, footer and system-health copy. Remaining dashboard/email/operator surfaces and browser/deployed evidence stay open.
