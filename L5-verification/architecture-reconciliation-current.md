...
| Network and per-model egress (F-02/F-04/F-43) | M990 adds verified TLS proxying, exact IPv4/IPv6 namespace firewall policy, capability-free sidecars, drain-safe health checks and continuous monitoring. M998 additionally runs the real Node shared-fetch guard in a disposable model namespace: matching unprivileged runner passes; initial namespace is rejected before fetch construction. Full evidence and exact open gates: [egress runtime acceptance](egress-runtime-acceptance.md). | Still partial: production privilege-separated provisioning; installed Node connector/MCP confinement and UDS dispatch; explicit direct-mode deployed dispatch; real endpoint/DNS and restart/credential-rotation acceptance; persisted tenant/health and queue/Sev-1/Relay behavior; deployed Network page/operator acceptance. The isolated Docker result is not proof these entire features are complete. |
...
| Scraper and research (F-17/F-18) | Authenticated bounded scrape runs, worker dispatch, model egress binding and partial-result/error handling exist. M577 aligns the durable `scrape_run` state, worker persistence and authenticated projection so mixed results remain `partial` end to end. M644 localizes the mounted result/refresh/history UI across all six launch locales with locale-aware counts and UTC timestamps while preserving authored/provider data and mutation semantics. M787 localizes the authenticated scraper route title and load-failure shell through the same catalog. M1011 exposes up to 24 time-ordered public-count observations per benchmark profile in an authorized model view and localizes the history labels and UTC timestamps. | Deployed sidecar/provider isolation, migration application, browser/mobile and result-quality acceptance. |
| Viral loop (F-79–F-86) | Metric/evidence filtering, publication-bound recipe evidence (hook, scheduled/actual time, bounded shoot controls, media format, ToS verdict and asset/hash-bound Rust vision descriptors), labels, recipes, embeddings/retrieval, and model-scoped weekly insight scheduling/card dispatch exist. M1013 preserves connector-declared provider metric dimensions, adds a latest-snapshot per-post view of tracked clicks and signed Fanvue subscription/PPV/refund outcomes, and feeds linked click/conversion outcomes into the recency-decayed contextual learner. M931 adds model-owned insight cards from published provider evidence; M989 adds durable source-card-to-Relay dispatch with stored-versus-delivery state. F-86 same-org opt-in sharing remains structurally bounded and tenant-scoped. | Disposable-PostgreSQL acceptance of M1013 and the learner query, migration application to any deployed database, external provider/runtime receipts and operator acceptance. Missing provider dimensions, unique visitors and causal lift are not inferred. |
| Connectors and OAuth (F-03/F-31/F-58–F-67) | **Source-wired/partial:** model-scoped OAuth/manual onboarding is mounted for Fanvue, TikTok, X, YouTube, Reddit, Instagram, Facebook, Discord, Threads and Snapchat with encrypted persistence and provider-derived capabilities. Model-egress refresh exists for TikTok/X/YouTube/Reddit plus provider-specific Fanvue and Snapchat rotation; disconnect revokes before local removal. The ten connector contracts implement provider-specific publish, metrics, moderation/community, Fanvue vault/inbox and asset operations only where their capability matrices advertise them. TikTok direct-vs-draft, YouTube Shorts/caption/thumbnail, Fanvue MCP upload/post/analytics/inbox/vault, and Snapchat Public Profile versus manual-assist Relay behavior are covered by source tests and UI wiring. | Live provider OAuth/refresh/revoke receipts, real upload/publish/metrics/moderation receipts, deployed migrations/RLS, browser acceptance and hosted runtime evidence remain open. Snapchat manual-assist is the truthful fallback where automatic organic posting is unavailable. |
...
| Link-in-bio (F-48–F-53) | The Native provider is the current production-enabled default. M1012 wires the authenticated model-scoped published-post selector, audited per-post short-link creation, public `/linkbio/:modelId/s/:slug` redirect, first-party click recording and attribution-report post-target projection into the dashboard. Generated UTMs carry the platform and published post-target ID; Fanvue events and recorded campaign costs use the existing attribution joins. | Fanlynks, Linktree and Beacons remain planned adapters and must stay hidden/rejected until their real lifecycle exists. Cross-provider normalization, deployed migration/RLS/runtime and browser acceptance remain separate; a database row is not evidence of an external connection. |
