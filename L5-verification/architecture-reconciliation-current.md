...
Source checkpoint: `f90e13d5d5e17f2f96c242ca393ac07f7b4e32a0`
...
| Team and shifts (F-24/F-25/F-26) | RBAC, shift lifecycle, handoff/post-note routes and dashboard controls exist. The dual-actor Chatter source slice adds human/LLM shift records, active-shift/agent-permission checks, durable roleplay handoffs, bounded memory and revisioned `soul.md` persona storage/API/UI. Team history now has bounded keyset pagination with dashboard older-history controls; Chatter roleplay uses its authorized personal shift roster rather than the administrative team endpoint. M667 adds a shared Grok roleplayer runtime and private assigned-LLM inbox drafting that reuses the pending-reply review flow, persists persona/memory provenance, and requires explicit human approval before send. | Authenticated multi-user browser/RLS acceptance, Grok roleplay and draft provider receipts, live migration/runtime acceptance and operator acceptance remain open. |
