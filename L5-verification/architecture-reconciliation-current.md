...
| Viral loop (F-79–F-86) | Metric/evidence filtering, publication-bound recipe evidence (hook, scheduled/actual time, bounded shoot controls, media format, ToS verdict and asset/hash-bound Rust vision descriptors), labels, recipes, embeddings/retrieval, parts of reward/digest logic, and a typed Relay-card lifecycle distinguishing durable `stored` evidence from external-dispatch states exist. M931 adds a model-scoped `viral.insight` job, localized analytics enqueue control, and model-owned Relay-card history that remains visible when no content bundle is present. Its card renderer is evidence-only and reads published provider snapshots with bounded group evidence. M989 adds the queued source-card-to-Relay dispatch path, shared binding preflight, per-binding pending markers and a nullable-bundle uniqueness guard, while preserving the stored-vs-provider-delivery distinction. | Revenue/conversion attribution, all contextual arms, cross-model opt-in behavior, recurring insight scheduling, migration application, provider/runtime and operator acceptance. M989 is source-level dispatch behavior only and does not claim a provider receipt or live migration. |
