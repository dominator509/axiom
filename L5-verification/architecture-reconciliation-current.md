...
| Network and per-model egress (F-02/F-04/F-43) | M990 adds verified TLS proxying, exact IPv4/IPv6 namespace firewall policy, capability-free sidecars, drain-safe health checks and continuous monitoring. Isolated Linux acceptance passes 64 real-kernel/socket tests; 59 focused TS tests pass. Full evidence and exact open gates: [egress runtime acceptance](egress-runtime-acceptance.md). | Still partial: production privilege-separated provisioning; OS confinement of Node connector/MCP callers; explicit direct-mode sync/dispatch; real endpoint/DNS and restart/credential-rotation acceptance; persisted tenant/health and queue/Sev-1/Relay behavior; deployed Network page/operator acceptance. The isolated sidecar result is not proof these entire features are complete. |
