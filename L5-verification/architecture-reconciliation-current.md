...
| Viral loop (F-79–F-86) | Metric/evidence filtering, publication-bound recipe evidence (hook, scheduled/actual time, media format and ToS verdict), labels, recipes, embeddings/retrieval and parts of reward/digest logic exist. | Thumbnail/shoot metadata, revenue/conversion attribution, all contextual arms, cross-model opt-in behavior, scheduled insight/Relay delivery and runtime acceptance. |
