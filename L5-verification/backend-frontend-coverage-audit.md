...
6. **Localization remains only partially cross-cutting.** The dashboard shell/navigation/settings, mobile selector/dashboard and persisted locale precedence now share a typed catalog, but remaining dashboard pages, auth, email and operator errors still need adoption plus a full date/number/currency formatting audit.
