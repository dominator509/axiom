...
| F-89 | **Source-wired/partial**: shared six-locale catalog and normalization, persisted user/org preference API, dashboard provider/navigation/settings wiring, authenticated shell workspace/home/role/pending/footer/system-health copy, login hero/form labels/errors/session advice, accessible `lang`, and mobile selector/dashboard labels now consume the same catalog; authored content remains separate | Remaining dashboard/email/operator adoption, every date/number/currency formatting surface, browser/mobile acceptance, and deployed migration/RLS/runtime evidence are open |
