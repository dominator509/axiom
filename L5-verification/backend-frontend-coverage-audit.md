...
6. **Localization remains only partially cross-cutting.** The dashboard shell/navigation/settings, authenticated login, mobile selector/dashboard and persisted locale precedence now share a typed catalog, but remaining dashboard pages, email and operator errors still need adoption plus a full date/number/currency formatting audit.
