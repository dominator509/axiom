...
const focusedVariants = process.argv[3] === '--variant-performance';
const focusedRetrieval = process.argv[3] === '--viral-retrieval';
const focusedPlaybook = process.argv[3] === '--playbook-history';
const focused = focusedWorker || focusedVariants || focusedRetrieval || focusedPlaybook;
...
try {
  ...
  const testFile = focusedPlaybook ? 'packages/api/src/routes/playbook-history.integration.test.ts' : focusedRetrieval ? 'packages/worker/src/viral-retrieval.integration.test.ts' : focusedVariants ? 'packages/api/src/routes/variant-performance.integration.test.ts' : 'packages/worker/src/worker-media.integration.test.ts';
